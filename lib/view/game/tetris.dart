...
class _TetrisState extends ConsumerState<Tetris> {
                  ...
                  Column(
                    children: [
                      CommonWidget.imageBuilder(
                        imageUrl: 'assets/svg/tetrisLogo.svg',
                                ...
                                Expanded(
                                  child: Padding(
                                    ...
                                    child: ElevatedButton(
                                        onPressed: () {
                                          String leftBrick =
                                              currentBrick[0].toString();
                                          if (leftBrick.substring(
                                                  leftBrick.length - 1,
                                                  leftBrick.length) !=
                                              "1") {
                                            if (!(bricks[randInt][pos]!.any(
                                                (element) => ocupiedPos
                                                    .contains(element - 1)))) {
                                              brickDown("left");
                                            }
  ...
  rotate() {
    ...
    if (zeroBrick.substring(zeroBrick.length - 1, zeroBrick.length) != "1" &&
        ...
        secondBrick.substring(secondBrick.length - 1, secondBrick.length) !=
            "1" &&
        secondBrick.substring(secondBrick.length - 1, secondBrick.length) !=
            "0" &&
  ...
  checkClearRow() {
    ...
    for (int i = 1; i <= 20; i++) {
      if (checkRow((i * 10) - 9, i * 10)) {
        ocupiedPos.removeRange(
            ocupiedPos.indexOf((i * 10) - 9), ocupiedPos.indexOf(i * 10));
        ocupiedPos.removeAt(ocupiedPos.indexOf(i * 10));

        ref.read(scoreProvider.notifier).state =
            ref.read(scoreProvider.notifier).state + 100;
